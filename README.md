...
4. **Export** — the right-hand panel always shows the current output. Switch between **CSS variables** and **Tailwind config**, copy it, download the file, or use **Download all (zip)** to get both plus a `/fonts` folder for any linked font files. With **Fluid `clamp()` sizing** on, every size exports as a `clamp()` that scales between your mobile and desktop values across the chosen viewport range.
